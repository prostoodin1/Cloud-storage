...
ARTICLES = (
    ...
    KnowledgeArticle(
        ...
        "Core открывает отдельный HTTPS-порт с белым списком клиентских файловых маршрутов; административный "
        "API и документация остаются только на 127.0.0.1. Рекомендуется VPN. Программа не меняет роутер и "
        "firewall автоматически: прямое перенаправление порта настраивает администратор вручную. Для нового "
        "устройства отдельно и временно разрешите удалённое подключение по коду, а затем снова выключите его.",
        "интернет удалённый доступ порт роутер outside",
    ),
    KnowledgeArticle(
        "multiple-servers",
        "Подключение",
