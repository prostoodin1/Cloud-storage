...
def pair(client, code):
    response = client.post(
        "/v1/web/pair", json={"code": code}, headers={"Origin": "http://testserver"}
    )
...
def test_browser_files_csrf_logout_and_manager_boundary(web):
    ...
    assert (
        client.put(base + "/files/example.txt", content=b"hello", headers=headers).status_code
        == 201
    )
    ...
    assert (
        client.post(
            base + "/directories", json={"logical_path": "Folder"}, headers=headers
        ).status_code
        == 201
    )
    assert (
        client.post(
            ...
            json={
                ...
                "destination_path": "Folder/Renamed.txt",
                "kind": "file",
            },
            headers=headers,
        ).status_code
    ...
    assert (
        client.post("/v1/web/logout", headers={"Origin": "https://attacker.invalid"}).status_code
        == 403
    )
...
def test_trusted_client_can_set_simple_password_without_reentering_old_one(web):
    _, client, code = web
    paired = client.post(
        "/v1/pairing/redeem",
        json={"code": code, "device_name": "Ноутбук", "platform": "Windows"},
    ).json()
    device = {"Authorization": f"Bearer {paired['device_token']}"}
    changed = client.patch(
        "/v1/account",
        headers=device,
        json={"username": "simple.login", "new_password": "1"},
    )
    assert changed.status_code == 200, changed.text
    login = client.post(
        "/v1/web/login",
        headers={"Origin": "http://testserver"},
        json={"username": "simple.login", "password": "1", "remember": False},
    )
    assert login.status_code == 200, login.text
...
def test_invalid_code_and_cookie_rejected(web):
    ...
    assert (
        client.post(
            "/v1/web/pair", json={"code": bad}, headers={"Origin": "http://testserver"}
        ).status_code
        == 422
    )
...
def test_browser_access_can_be_revoked(web, revocation):
    ...
    with runtime.database.transaction() as connection:
        if revocation == "device":
            connection.execute(
                "UPDATE devices SET status='revoked' WHERE id=?", (result["device"]["id"],)
            )
        elif revocation == "user":
            connection.execute(
                "UPDATE users SET enabled=0 WHERE id=?", (result["device"]["user_id"],)
            )
...
def test_each_file_capability_is_enforced_for_cookies(web):
    ...
    space = repo.create_shared_space(
        name="Shared",
        quota_bytes=1024**3,
        primary_storage_root_id=None,
        fallback_storage_root_id=None,
    )
    user_id = result["device"]["user_id"]
    base = f"/v1/spaces/{space.id}"
    repo.set_space_member(
        space.id,
        user_id,
        {"read": True, "upload": True, "modify": True, "delete": True, "share": True},
    )
    assert (
        client.put(base + "/files/a.txt", content=b"original", headers=headers).status_code == 201
    )
    ...
    assert (
        client.post(
            base + "/directories", json={"logical_path": "denied"}, headers=headers
        ).status_code
        == 403
    )
    assert (
        client.post(
            base + "/moves",
            json={"source_path": "a.txt", "destination_path": "b.txt", "kind": "file"},
            headers=headers,
        ).status_code
        == 403
    )
...
def test_external_browser_cookie_secure_and_remote_pairing_switch(tmp_path, monkeypatch):
    ...
    config = CoreConfig(
        data_directory=tmp_path, zrok_enabled=True, remote_pairing_enabled=True, zrok_port=18768
    )
    ...
    with TestClient(app, base_url="https://cloud.example:18768") as remote:
        ...
        response = remote.post(
            "/v1/web/pair", json={"code": code}, headers={"Origin": "https://cloud.example:18768"}
        )
        assert response.status_code == 200, response.text
        ...
        assert (
            remote.post(
                "/v1/web/pair",
                json={"code": code},
                headers={"Origin": "https://cloud.example:18768"},
            ).status_code
            == 403
        )
