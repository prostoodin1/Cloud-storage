...
ARTICLES = (
    ...
    KnowledgeArticle(
        ...
        "В расширенном режиме откройте «Автоматизация», выберите условие, действие и cooldown. "
        "Правило хранится в SQLite Core, переживает перезапуск и оставляет историю каждого запуска. "
        "Доступны только встроенные действия: уведомление, быстрая проверка и защитный режим только "
        "чтения. Команды, скрипты и произвольный код правила не запускают.",
        "автоматизация правило триггер действие cooldown расписание срабатывание",
    ),
    KnowledgeArticle(
