...
class ControlCenterPage(QWidget):
    ...
    open_container_manager_requested = Signal()
    ...
    def _build_cell_tab(self) -> QWidget:
        ...
        open_manager = QPushButton("Открыть отдельный менеджер контейнеров")
        open_manager.setProperty("primary", True)
        open_manager.clicked.connect(self.open_container_manager_requested)
        layout.addWidget(open_manager)
        form = QFormLayout()
