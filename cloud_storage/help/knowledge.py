...
ARTICLES = (
    ...
    KnowledgeArticle(
        ...
        "Откройте вкладку «Обновления», нажмите «Проверить», затем скачайте и установите новую "
        "версию. Приложение проверяет HTTPS, Ed25519-подпись манифеста, размер и SHA-256 установщика "
        "и повторяет проверку прямо перед запуском. Обновление заменяет только программу: серверная "
        "база, пользователи, файлы, клиентские профили и токены сохраняются.",
        "обновление новая версия скачать установить подпись sha данные сохранятся",
    ),
    KnowledgeArticle(
        "uninstall-data",
        "Обновления",
